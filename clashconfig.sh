...
dnsinclash=$(get merlinclash_dnsinclash)
...
check_coremark(){
	...
	if [ "${recordbycron}" != "1" ];then
		if [ "${mounted_nu}" -eq "2" ]; then
			coremark="1"
            echo_date "检测到已安装插件usb2jffs并成功挂载，将使用Clash内核内置代理组状态保存服务" >> $LOG_FILE
        elif [ "${LINUX_VER}" -gt "41" ]; then
            coremark="1"
            echo_date "检测到您的路由Linux内核版本大于4.19，将使用Clash内核内置代理组状态保存服务" >> $LOG_FILE
        else
	...
	if [ "$clashTmpV1" = "Meta" ];then
		merlinclash_clash_version_tmp="Mihomo $clashTmpV2"; 
	else
...
write_setmark_cron_job(){
	if [ "${coremark}" == "1" ]; then
		echo_date "Linux内核版本大于4.19或者启用了jffs2usb，使用Clash内核内置代理组状态保存服务" >> $LOG_FILE
		echo_date "Linux内核版本大于4.19或者启用了jffs2usb，使用Clash内核内置代理组状态保存服务" > /tmp/upload/merlinclash_node_mark.log
...
start_remark(){
	...
	/bin/sh /koolshare/scripts/clash_patchmode.sh >/dev/null 2>&1
}
set_sys() {
	# set_ulimit
	ulimit -n 16384
...
restart_dnsmasq() {
	...
	if  [ "$mcenable" = "1" ] && [ "$dnsinclash" = "1" ] ; then
		if [ "$dnsplan" == "rh" ]; then
			cat >/etc/resolv.conf <<-EOF
				nameserver 127.0.0.1
			EOF
		elif [ -n "$LOCAL_DNSISP_DNS1" ]; then
			cat >/etc/resolv.conf <<-EOF
...
start_custom(){
  ...
  #内核代理组状态记忆&fake-ip缓存启用
	...
	#SNIFFER---
	#移出启动配置文件 http/socks 端口
	if [ "$merlinclash_mixport_enable" == "0" ] ; then
		echo_date "Http/Socks5代理端口未开启" >> $LOG_FILE
		sed -i "s/^port: 3333/ /g" $yamlpath
		sed -i "s/^socks-port: 23456/ /g" $yamlpath
	fi
	#Geo数据库写入
	if [ "$merlinclash_geoip_type" == "full" ] ; then
		echo_date "修改GeoIP数据库为：Loyalsoldier-GeoIP-Full" >> $LOG_FILE
		# 强制替换整行为 Loyalsoldier 的 GeoIP 地址
		sed -i 's|geoip: ".*"|geoip: "https://testingcf.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/geoip.dat"|g' $yamlpath
	else
		echo_date "修改GeoIP数据库为：Loyalsoldier-GeoIP-Lite" >> $LOG_FILE
		# 即使没选 Full，也强制使用 Loyalsoldier 的加速地址（保证成功率）
		sed -i 's|geoip: ".*"|geoip: "https://testingcf.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/geoip.dat"|g' $yamlpath
	fi
if [ "$merlinclash_geosite_type" == "lite" ] ; then
		echo_date "修改GeoSite数据库为：Loyalsoldier-GeoSite-Lite" >> $LOG_FILE
		# 使用 jsDelivr 加速下载 Loyalsoldier 的 geosite-lite
		sed -i 's|geosite: ".*"|geosite: "https://testingcf.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/geosite.dat"|g' $yamlpath
	elif [ "$merlinclash_geosite_type" == "full" ] ; then
		echo_date "修改GeoSite数据库为：Loyalsoldier-GeoSite-Full" >> $LOG_FILE
		# 同样替换为 Loyalsoldier 的完整版地址
		sed -i 's|geosite: ".*"|geosite: "https://testingcf.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/geosite.dat"|g' $yamlpath
	else
		echo_date "使用默认地址：尝试匹配并替换为 Loyalsoldier" >> $LOG_FILE
		# 兜底逻辑：强制将配置中原本那个 flyhigherpi 的地址改为 Loyalsoldier
		sed -i 's|geosite: "https://testingcf\.jsdelivr\.net/gh/flyhigherpi/.*"|geosite: "https://testingcf.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/geosite.dat"|g' $yamlpath
	fi
	...
	if [ "$mcc" == "1" ]; then
		...
		if [ "$merlinclash_mixport_enable" == "1" ] ; then
			...
			echo_date "Http/Socks5代理端口未开启，不修改" >> $LOG_FILE
	    fi
